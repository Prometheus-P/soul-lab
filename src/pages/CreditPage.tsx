...
export default function CreditPage() {
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      const [productsData, balanceData] = await Promise.all([getProducts(), getBalance(userKey)]);
  ...
  const handlePurchase = async (product: CreditProduct) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        await loadData();
        toast(`${product.totalCredits} 복채이 충전되었습니다!`, 'success');
      } else {
    ...
    return (
      <div className="container">
        ...
        <div style={{ textAlign: 'center', padding: 40 }}>
          <div
            style={{ fontSize: 48, marginBottom: 16, animation: 'pulse 2s ease-in-out infinite' }}
          >
            💫
          </div>
  ...
  if (viewMode === 'purchasing' && selectedProduct) {
    return (
      <div className="container">
        ...
        <div style={{ textAlign: 'center', padding: 40 }}>
          <div
            style={{ fontSize: 64, marginBottom: 24, animation: 'pulse 1.5s ease-in-out infinite' }}
          >
            ✨
          </div>
          ...
          <p className="p" style={{ color: 'rgba(255,255,255,0.7)', marginBottom: 8 }}>
            {selectedProduct.totalCredits} 복채
          </p>
  ...
  return (
    <div className="container">
      <Header title="💎 복채 충전" subtitle="AI 상담에 필요한 복채을 충전하세요" />
      ...
      <div
        ...
        style={{
          background:
            'linear-gradient(135deg, rgba(147, 112, 219, 0.3) 0%, rgba(255, 215, 0, 0.15) 100%)',
      ...
      >
        ...
        <div className="glow-text" style={{ fontSize: 36, fontWeight: 700 }}>
          {balance?.credits ?? 0} <span style={{ fontSize: 18 }}>복채</span>
        </div>
      ...
      <div className="footer">* 복채은 환불되지 않습니다. 신중하게 구매해주세요.</div>
    </div>
  );
}
